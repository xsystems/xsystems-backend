...
public class AuthenticationServiceImpl implements AuthenticationService {
  ...
  public User authenticate(final String authorizationHeaderString) throws AuthenticationException {
    ...
    if (!authorizationHeaderString.startsWith(BASIC)) {
      throw new AuthenticationException(
              "The '" + HttpHeaders.AUTHORIZATION + "' header its value should start with '"
                  + BASIC + "'.");
    }
    ...
    if (values.length != 2) {
      throw new AuthenticationException(
              "Unable to obtain credentials from the '" + HttpHeaders.AUTHORIZATION
                  + "' header its value.");
    }
  ...
  private String hashPassword(final char[] password, final byte[] salt, final int iterations,
                              final int keyLength)
  ...
  private boolean validate(final User user, final String password)
    ...
    final String passwordHash = hashPassword(password.toCharArray(), salt, iterations,
